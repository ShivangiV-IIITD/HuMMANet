...
GMMC_normalize_extdata_dir <- function(extdata_dir = NULL) {
...
GMMC_read_csv_any <- function(path) {
  if (!file.exists(path)) {
...
#' Study Index for Gut-Microbiome–Metabolome-Compendium
#'
...
#' @export
GMMC_study_index <- function(extdata_dir = NULL) {
  extdata_dir <- GMMC_normalize_extdata_dir(extdata_dir)
...
#'
#' @inheritParams GMMC_study_index
#'
#' @return Character vector of study identifiers.
#' @export
GMMC_studies <- function(extdata_dir = NULL) {
  unique(GMMC_study_index(extdata_dir = extdata_dir)$study)
}
...
#' @export
GMMC_available_modalities <- function(study, extdata_dir = NULL) {
  index <- GMMC_study_index(extdata_dir = extdata_dir)
...
GMMC_load_modality <- function(
  ...
  path <- file.path(GMMC_normalize_extdata_dir(extdata_dir), rel_path)
  GMMC_read_csv_any(path)
}
...
GMMC_load_study <- function(
  ...
  for (modality in modalities) {
    loaded[[modality]] <- GMMC_load_modality(
      study = study,
      modality = modality,
...
#' Gut-Microbiome–Metabolome-Compendium Accessor
#'
#' Main convenience loader for Gut-Microbiome–Metabolome-Compendium data.
#'
#' @inheritParams GMMC_study_index
