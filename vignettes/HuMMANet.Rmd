...
```{r connect-hub, eval = FALSE}
hub <- HuMMANetHub()
hub
```

In a published Bioconductor release, `HuMMANetHub()` is the main entry point to
the remote resources. During package development before the public
`ExperimentHub` records are registered, the listing examples below can still be
demonstrated from package metadata or local development files.
...
contains `inst/extdata/studies`. To keep package checks stable before the
records are formally registered in `ExperimentHub`, those chunks are shown but
only evaluated when local study files are available.

## Load one study

```{r load-study, eval = hummanet_has_local_studies}
...
```{r inspect-structure, eval = hummanet_has_local_studies}
...
```{r load-metadata, eval = hummanet_has_local_studies}
...
```{r original-profile, eval = hummanet_has_local_studies}
...
```{r harmonized-profile, eval = hummanet_has_local_studies}
...
```{r taxa-profile, eval = hummanet_has_local_studies}
...
```{r pathway-annotations, eval = hummanet_has_local_studies}
...
```{r study-metadata-slot, eval = hummanet_has_local_studies}
...
```{r assay-matrices, eval = hummanet_has_local_studies}
...
```{r subset-study, eval = hummanet_has_local_studies}
...
```{r multiple-studies, eval = hummanet_has_local_studies}
